...
<head>
  ...
  <style>
    ...
    /* Modal */
    #help-modal, #modal { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.9); flex-direction: column; justify-content: center; align-items: center; z-index: 99; }
...
<body>
  ...
  <div class="header">
    ...
    <button class="btn-nav" onclick="toggleHelp()">❓ Help</button>
  </div>
  ...
  <div id="help-modal" onclick="toggleHelp()">
    <div class="box">
      <h3 style="color:#00cec9; margin-top:0; text-align:center;">CÁCH CHƠI</h3>
      <ul style="color:#ccc; padding-left:20px;">
        <li>Chờ vòng tròn đổi màu.</li>
        <li>Bấm nút màu tương ứng <b>nhanh nhất</b>.</li>
        <li>Đúng: +1 điểm.</li>
        <li>Sai: -1 điểm.</li>
      </ul>
      <button class="btn-nav" style="width:100%; text-align:center;">OK</button>
    </div>
  </div>
...
<script>
  ...
  ws.onmessage = (evt) => {
    ...
    if(msg.startsWith("reflex:")) {
      let parts = msg.split(':');
      if(parts[1] === 'welcome') { mySlot = parseInt(parts[2]); document.getElementById('status').innerText = "Bạn là P" + (mySlot+1); }
        ...
        document.getElementById('target-box').style.background = c[parseInt(parts[2])];
  ...
  function clickColor(id) { if(mySlot!=-1) ws.send('reflex:'+id); }
